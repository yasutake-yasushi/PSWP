...
export async function getContractItem(id: string): Promise<ContractItem> {
...
export async function updateContractItem(id: string, input: ContractItemInput): Promise<ContractItem> {
  const res = await fetch(`${BASE_URL}/api/contractitems/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(input),
